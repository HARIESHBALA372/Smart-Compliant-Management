...
export function ManagerComplaintsPage() {
  ...
  useEffect(() => {
    dispatch(fetchComplaints({ page: 1, limit: 10, search: debouncedSearch || undefined }))
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Monitor all complaints across the organization</p>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={6}>
                  <EmptyState icon={<FileText className="h-12 w-12" />} title="No complaints found" />
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <Pagination
        ...
        onPageChange={(p) => dispatch(fetchComplaints({ page: p }))}
      />
    </div>
  )
}